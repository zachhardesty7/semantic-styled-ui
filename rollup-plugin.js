...
export default function propTypesFromTS() {
  return {
    ...
    writeBundle() {
      (async() => {
        const tsOptions = ttp.loadConfig(path.resolve(__dirname, './tsconfig.json'))
        ...
        testCases.forEach(async(testCase) => {
          if (!testCase.includes('index')) {
            ...
            const ast = ttp.parseFromProgram(testCase, program, options)

            let output = fs.readFileSync(outputPath, 'utf8')

            const result = ttp.generate(ast, options)
